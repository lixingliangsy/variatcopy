    ...
    <Layout>
      ...
      <div className="max-w-2xl mx-auto text-center mb-10">
        <div className="flex flex-col sm:flex-row gap-3 justify-center items-center">
          <a href="/api/checkout" className="inline-block bg-brand text-white font-semibold px-8 py-4 rounded-xl hover:opacity-90">
            {`Monthly — ${PRODUCT.priceMonthly}/mo`}
          </a>
          <a href="/api/checkout?cycle=yearly" className="inline-block border border-brand text-brand font-semibold px-8 py-4 rounded-xl hover:opacity-90">
            {`Yearly — ${PRODUCT.priceYearly}/yr`}
          </a>
        </div>
        <p className="text-sm text-gray-500 mt-3">Cancel anytime. Save 2 months with yearly. Secure checkout via Waffo.</p>
      </div>
